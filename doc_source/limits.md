...
+ VPC peering within a single AWS Region when using [certain Amazon EC2 instance types](manage-fs-access-change-vpc.md#manage-fs-access-vpc-peering) is supported\. However, inter\-region VPC peering and VPC peering within an AWS Region using other instance types are not supported\.