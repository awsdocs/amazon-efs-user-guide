...
+ [Troubleshooting Mount Issues](#troubleshooting-efs-mounting)
...
+ [Troubleshooting Encrypted\-at\-Rest File Systems](#troubleshooting-efs-encryption)
...
To enable encryption of data in transit for your Amazon EFS file system, Transport Layer Security \(TLS\) is enabled when you mount your file system using the Amazon EFS mount helper\.

**Action to Take**  
If mounting with encryption of data in transit fails, take the following steps:

1. If possible, use the Amazon EFS mount helper, with the default configuration settings\. For more information on the mount helper, see [EFS Mount Helper](using-amazon-efs-utils.md#efs-mount-helper)

1. Make sure that DNS hostnames are enabled in your Amazon VPC\. For more information, see [Using DNS with Your VPC](http://docs.aws.amazon.com/AmazonVPC/latest/UserGuide/vpc-dns.html) in the Amazon VPC User Guide\.

1. Confirm that your file system ID is correct\.

1. Make sure that your client's operating system has the proper dependencies and versions for Python, stunnel, and the nfs client\.

After you've taken those steps, try the mount command again\. If it fails again, contact AWS Support\.
...
## Troubleshooting Encrypted\-at\-Rest File Systems<a name="troubleshooting-efs-encryption"></a>
